...
public abstract class StratumConnection implements Connection {
  ...
  /**
   * Return the output stream of the connection. If it is not available, then throw an exception.
   * 
   * @return
   * @throws Exception
   */
        ...
        if (request.getId() != null) {
          ...
          if (request.getMethod() != null) {
            ...
            if (request != null) {
              ...
              LOGGER.debug("Drop response since no request has been sent with the id {}.",
                  response.getId());
            }
          }
        } else {
    ...
    switch (notification.getMethod()) {
      ...
      case MiningSetExtranonceNotification.METHOD_NAME:
        MiningSetExtranonceNotification setExtranonce =
            new MiningSetExtranonceNotification(notification);
      ...
      default:
        LOGGER.warn("Unknown notification type on connection {}. methodName: {}, params: {}",
            getConnectionName(), notification.getMethod(), notification.getParams());
        break;
    }
  }
  ...
  protected void onResponseReceived(JsonRpcRequest request, JsonRpcResponse response) {
    switch (request.getMethod()) {
      ...
      case MiningExtranonceSubscribeRequest.METHOD_NAME:
        MiningExtranonceSubscribeRequest subscribeExtranonceRequest =
            new MiningExtranonceSubscribeRequest(request);
        MiningExtranonceSubscribeResponse subscribeExtranonceResponse =
            new MiningExtranonceSubscribeResponse(response);
      ...
      default:
        LOGGER.warn("Unknown response type on connection {}. methodName: {}, result: {}",
            getConnectionName(), request.getMethod(), response.getResult());
        break;
    }
  }
  ...
  protected void onRequestReceived(JsonRpcRequest request) {
    switch (request.getMethod()) {
      ...
      case MiningExtranonceSubscribeRequest.METHOD_NAME:
        MiningExtranonceSubscribeRequest extranonceSubscribeRequest =
            new MiningExtranonceSubscribeRequest(request);
      ...
      case MiningGetTransactionsRequest.METHOD_NAME:
        MiningGetTransactionsRequest getTransactionsRequest =
            new MiningGetTransactionsRequest(request);
      ...
      default:
        LOGGER.warn("Unknown request type on connection {}. methodName: {}, id: {}, params: {}",
            getConnectionName(), request.getMethod(), request.getId(), request.getParams());
        break;
    }
  }
   ...
   */
  protected abstract void onExtranonceSubscribeResponse(MiningExtranonceSubscribeRequest request,
      MiningExtranonceSubscribeResponse response);

  /**
   * Called when a authorize response is received
   */
  protected abstract void onAuthorizeResponse(MiningAuthorizeRequest request,
      MiningAuthorizeResponse response);

  /**
   * Called when a subscribe response is received
   */
  protected abstract void onSubscribeResponse(MiningSubscribeRequest request,
      MiningSubscribeResponse response);

  /**
   * Called when a submit response is received
   */
  protected abstract void onSubmitResponse(MiningSubmitRequest request,
      MiningSubmitResponse response);

  /**
   * Called when a client getVersion response is received
   */
  protected abstract void onGetVersionResponse(ClientGetVersionRequest request,
    ...
    if (socket != null) {
      if (socket.getRemoteSocketAddress() instanceof InetSocketAddress) {
        ...
        if (socket.getRemoteSocketAddress() != null)
          ...
          name = "Test-" + new Random().nextInt(10);
      }
    }
    return name;
  }
