...
public class AuthorizationManager {
  ...
  /**
   * Check if the given user and password are authorized to connect. Throws an exception if the user
   * is not authorized.
   * 
   * @param username
   * @param password
  ...
  public void checkAuthorization(WorkerConnection connection, MiningAuthorizeRequest request)
    ...
    if (!isPresentUserInDB(request.getUsername())) {
      ...
      throw new AuthorizationException(
          "The address " + connection.getRemoteAddress() + " is banned.");
    }
  }

  public boolean isPresentUserInDB(String userName) {
  ...
  /**
   * Ban the given address until the next proxy restart. The address can be an IP (v4 or v6) or a
   * hostname.
   * 
   * @param username
  ...
  /**
   * Unban the given address. The address can be an IP (v4 or v6) or a hostname.
   * 
   * @param username
   * @throws UnknownHostException
   */
  public void unbanAddress(AddressDTO address) throws UnknownHostException, NotFoundException {
    ...
    if (!bannedAddresses.remove(inetAddress)) {
      throw new NotFoundException(
          "Address " + inetAddress.toString() + " not banned. Cannot unban.");
    }
  }
