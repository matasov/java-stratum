...
/**
 * An abstract PoolSwitchingStrategyManager that allows one pool max to be active at the same time.
 * 
 * @author Strat
 * 
 */
public abstract class MonoCurrentPoolStrategyManager implements PoolSwitchingStrategyManager {
  ...
  /**
   * Check if connections are bound to the good pool. If not, rebind the worker connection to the
   * pool with highest priority.
   */
