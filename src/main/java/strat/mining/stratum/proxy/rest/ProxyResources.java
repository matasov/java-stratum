...
import strat.mining.stratum.proxy.rest.dto.RegisterDTO;
...
public class ProxyResources {
  ...
  private strat.mining.stratum.proxy.manager.LogManager logManager =
      strat.mining.stratum.proxy.manager.LogManager.getInstance();
  ...
  public Response getAuthenticationDetails() {
    ...
    result.setAuthenticationNeededForWriteAccess(
        StringUtils.isNotBlank(ConfigurationManager.getInstance().getApiUser())
  ...
  @Path("user/list")
  @ApiOperation(
      value = "Get the list of users that has been connected at least once since the proxy is started.",
      response = UserDetailsDTO.class, responseContainer = "List")
  @PubliclyAvailable
  ...
  /**
   * Kick the given username. Kill all connections where the user has been seen (WARN: this may kill
   * connections supporting other users)
   * 
   * @return
   * 
   */
  ...
  @Path("user/kick")
  @ApiOperation(
      value = "Kick the given username. Kill all connections where the user has been seen (WARN: this may kill connections supporting other users)",
  ...
  public Response kickUser(UserNameDTO username) {
    ...
    try {
      ...
      status
          .setMessage("Failed to kick the user " + username.getUsername() + ". " + e.getMessage());
    }
  ...
  @Path("user/ban")
  @ApiOperation(
      value = "Ban the given username until the proxy restart. The user will not be authorized to reconnect.",
      response = StatusDTO.class)
  ...
  @GET
  @Path("user/ban/list")
  @ApiOperation(value = "List all banned users.", response = String.class,
  ...
  @ApiOperation(value = "Return the list of all worker connections.",
      response = WorkerConnectionDTO.class, responseContainer = "List")
  @PubliclyAvailable
  ...
  @ApiOperation(value = "Kill the connection with the given address and port.",
      response = StatusDTO.class)
  public Response kickConnection(ConnectionIdentifierDTO connection) {
    ...
    try {
      ...
      LOGGER.error("Failed to kick the connection with address {} and port {}.",
          connection.getAddress(), connection.getPort(), e);
      status.setStatus(StatusDTO.FAILED_STATUS);
      status.setMessage("Failed to kick the connection with address " + connection.getAddress()
          + " and port " + connection.getPort() + ". " + e.getMessage());
    }
  ...
  public Response banIp(AddressDTO address) {
    ...
    try {
      ...
      status.setStatus(StatusDTO.FAILED_STATUS);
      status.setMessage(
          "Failed to ban the address " + address.getAddress() + " connections. " + e.getMessage());
    }
  ...
  public Response kickAddress(AddressDTO address) {
    ...
    try {
      ...
      status.setStatus(StatusDTO.FAILED_STATUS);
      status.setMessage("Failed to kick the connections with address " + address.getAddress() + ". "
          + e.getMessage());
    }
  ...
  @ApiOperation(value = "List all banned addresses.", response = String.class,
      responseContainer = "List")
  ...
  @ApiResponses({
      @ApiResponse(code = 500, message = "Failed to start the pool.", response = StatusDTO.class),
  ...
  @ApiResponses({@ApiResponse(code = 404, message = "Pool not found.", response = StatusDTO.class),
      @ApiResponse(code = 400, message = "Bad parameter sent.", response = StatusDTO.class)})
  ...
  @ApiResponses({
      @ApiResponse(code = 500, message = "Failed to update the pool.", response = StatusDTO.class),
      @ApiResponse(code = 400, message = "Bad parameter sent.", response = StatusDTO.class),
      @ApiResponse(code = 404, message = "Pool not found.", response = StatusDTO.class)})
  ...
  @ApiResponses({
      @ApiResponse(code = 400, message = "Log level not known.", response = StatusDTO.class)})
  ...
  @POST
  @Path("hashrate/user")
  @ApiOperation(value = "Return the hashrate history of a user.", response = HashrateModel.class,
      responseContainer = "List")
  @PubliclyAvailable
  public Response getUserHashrateHistory(UserNameDTO username, @ApiParam(required = false,
      name = "start",
      value = "Allow to filter returned values on the given start timestamp (UTC from Epoch in seconds)") @QueryParam("start") Long startTimestamp,
      @ApiParam(required = false, name = "end",
          value = "Allow to filter returned values on the given end timestamp (UTC from Epoch in seconds)") @QueryParam("end") Long endTimestamp) {
    HashrateHistoryDTO result = new HashrateHistoryDTO();
    List<HashrateModel> userHashrates =
        databaseManager.getUserHashrate(username.getUsername(), startTimestamp, endTimestamp);
  ...
  @POST
  @Path("hashrate/pool")
  @ApiOperation(value = "Return the hashrate history of a pool.", response = HashrateModel.class,
      responseContainer = "List")
  @ApiResponses({
      @ApiResponse(code = 500, message = "Error during pool hashrate history response build."),
      @ApiResponse(code = 404, message = "Pool not found.")})
  @PubliclyAvailable
  public Response getPoolHashrateHistory(PoolNameDTO poolName, @ApiParam(required = false,
      name = "start",
      value = "Allow to filter returned values on the given start timestamp (UTC from Epoch in seconds)") @QueryParam("start") Long startTimestamp,
      @ApiParam(required = false, name = "end",
          value = "Allow to filter returned values on the given end timestamp (UTC from Epoch in seconds)") @QueryParam("end") Long endTimestamp) {
    ...
    if (pool != null) {
      try {
        List<HashrateModel> poolHashrates =
            databaseManager.getPoolHashrate(pool.getHost(), startTimestamp, endTimestamp);
  ...
  @POST
  @Path("log/since")
  @ApiOperation(value = "Return log message since the given timestamp.", response = LogEntry.class,
      responseContainer = "List")
  ...
  @GET
  @Path("summary")
  @ApiOperation(value = "Return a summary of the current state of the proxy.",
  ...
  @PubliclyAvailable
  public Response tryRegister(RegisterDTO register) {
    String errorMessage = "";
  ...
  @POST
  @Path("login")
  @ApiOperation(value = "User login.", response = RegisterDTO.class)
  @PubliclyAvailable
  ...
  private Level getLogLevel(String logLevel) throws BadParameterException {
    ...
    if ("FATAL".equalsIgnoreCase(logLevel)) {
      ...
      throw new BadParameterException("Unknown LogLevel " + logLevel
  ...
  private PoolDetailsDTO convertPoolToDTO(Pool pool) {
    ...
    result.setRejectedHashesPerSeconds(
        Double.valueOf(pool.getRejectedHashesPerSeconds()).longValue());
  ...
  private WorkerConnectionDTO convertWorkerConnectionToDTO(WorkerConnection connection) {
    ...
    result
        .setAcceptedHashesPerSeconds(Double.valueOf(connection.getAcceptedHashrate()).longValue());
    result
  ...
  private UserDetailsDTO convertUserToDTO(User user) {
    ...
    result.setLastShareSubmitted(
        user.getLastShareSubmitted() != null ? user.getLastShareSubmitted().getTime() : null);
