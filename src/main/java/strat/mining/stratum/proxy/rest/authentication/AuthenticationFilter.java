...
public class AuthenticationFilter extends BaseFilter {
  ...
  private static final Boolean API_READ_ONLY_ENABLED =
  ...
  public AuthenticationFilter(String basePath) {
    Reflections reflections = new Reflections(new ConfigurationBuilder()
        .setUrls(ClasspathHelper.forPackage(ProxyResources.class.getPackage().getName()))
    ...
    publiclyAvailablePathes
        .add(UriBuilder.fromPath("/").path("register.html").build().toASCIIString());
    publiclyAvailablePathes
        .add(UriBuilder.fromPath("/").path("customer.html").build().toASCIIString());
    publiclyAvailablePathes
        .add(UriBuilder.fromPath("/").path("login.html").build().toASCIIString());
    publiclyAvailablePathes
        .add(UriBuilder.fromPath("/").path("js/libs/jquery-2.1.3.js").build().toASCIIString());
    publiclyAvailablePathes
        .add(UriBuilder.fromPath("/").path("css/stratumProxy.css").build().toASCIIString());
    publiclyAvailablePathes
        .add(UriBuilder.fromPath("/").path("css/bootstrap-select.min.css").build().toASCIIString());
    publiclyAvailablePathes
        .add(UriBuilder.fromPath("/").path("css/bootstrap-theme.min.css").build().toASCIIString());
    publiclyAvailablePathes
        .add(UriBuilder.fromPath("/").path("css/bootstrap.min.css").build().toASCIIString());
    publiclyAvailablePathes
        .add(UriBuilder.fromPath(apiBasePath).path("register").build().toASCIIString());
    publiclyAvailablePathes
        .add(UriBuilder.fromPath(apiBasePath).path("login").build().toASCIIString());
    publiclyAvailablePathes
        .add(UriBuilder.fromPath(apiBasePath).path("customer").build().toASCIIString());
    if (API_READ_ONLY_ENABLED) {
      for (Method resource : resources) {
        Path annotation = resource.getAnnotation(Path.class);
        if (annotation != null) {
          publiclyAvailablePathes
              .add(UriBuilder.fromPath(basePath).path(resource).build().toASCIIString());
        }
      }
    }
  }
  ...
  public NextAction handleRead(FilterChainContext ctx) throws IOException {
    ...
    if (API_USER != null) {
      if (!API_READ_ONLY_ENABLED) {
        ...
        System.out.println("isPubliclyAvailable: " + isPubliclyAvailable);
        System.out.println(requestedUri + " ? " + (isPubliclyAvailable ? "present" : "not found"));
  ...
  private NextAction checkCredentials(FilterChainContext ctx, final HttpRequestPacket request) {
    ...
    try {
      ...
      if (API_USER.equals(credentials.getFirst())
          && (API_PASSWORD == null || API_PASSWORD.equals(credentials.getSecond()))) {
