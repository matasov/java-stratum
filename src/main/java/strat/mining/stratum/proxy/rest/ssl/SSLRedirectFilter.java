...
public class SSLRedirectFilter extends SSLBaseFilter {
  ...
  @Override
  public SSLTransportFilterWrapper createOptimizedTransportFilter(
  ...
  private NextAction redirectToHttps(FilterChainContext ctx, String rawRequest) throws IOException {
    System.out.println(SSLRedirectFilter.class+"::redirectToHttps ");
  ...
  public class HttpRequest {
    ...
    public HttpRequest(String rawRequest) throws IOException {
      ...
      if (splittedMethod.length != 3) {
        throw new IOException("Failed to parse the HTTP request " + rawRequest
            + ". METHOD line has only " + splittedMethod.length + " members.");
      }
