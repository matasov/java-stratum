...
public class GetworkRequestHandler extends HttpHandler {
  ...
  private GetWorkerConnectionRepository workerConnectionRepo =
  ...
  public GetworkRequestHandler() {
    ...
    this.workerConnections =
        Collections.synchronizedMap(new HashMap<InetAddress, GetworkWorkerConnection>());
  ...
  public void service(final Request request, final Response response) throws Exception {
    ...
    try {
      ...
      final GetworkRequest getworkRequest =
          jsonUnmarshaller.readValue(content, GetworkRequest.class);
      ...
      if (!request.getRequestURI().equalsIgnoreCase(Constants.DEFAULT_GETWORK_LONG_POLLING_URL)) {
        ...
        if (getworkRequest.getData() != null) {
          ...
          LOGGER.debug("New getwork submit request from user {}@{}.",
              request.getAttribute("username"), workerConnection.getConnectionName());
        ...
        LOGGER.debug("New getwork long-polling request from user {}@{}.",
            request.getAttribute("username"), workerConnection.getConnectionName());
  ...
  private void processLongPollingRequest(final Request request, final Response response,
    ...
    final LongPollingCallback longPollingCallback = new LongPollingCallback() {
      public void onLongPollingResume() {
        try {
          ...
          LOGGER.error("Failed to send long-polling response to {}@{}.",
              request.getAttribute("username"), workerConnection.getConnectionName(), e);
        }
      }
    ...
    response.suspend(70, TimeUnit.SECONDS, new CompletionHandler<Response>() {
      ...
      public void failed(Throwable throwable) {
        LOGGER.error("Long-polling request of {}@{} failed. Cause: {}",
            request.getAttribute("username"), workerConnection.getConnectionName(),
  ...
  protected void processGetworkRequest(Request request, Response response,
    ...
    LOGGER.debug("Returning response to {}@{}: {}", request.getAttribute("username"),
  ...
  protected void processGetworkSubmit(Request request, Response response,
    ...
    if (ConfigurationManager.getInstance().isValidateGetworkShares()) {
      if (ConfigurationManager.getInstance().isScrypt()) {
        isShareValid =
            !ScryptHashingUtils.isBlockHeaderScryptHashBelowTarget(getworkRequest.getData(),
                workerConnection.getGetworkTarget());
      } else {
        isShareValid =
            !SHA256HashingUtils.isBlockHeaderSHA256HashBelowTarget(getworkRequest.getData(),
                workerConnection.getGetworkTarget());
      }
    }
  ...
  /**
   * Check if the request is authorized. If not authorized, throw an exception. The response status
   * code and headers are modified.
   * 
   * @param request
...
//      }
  ...
  /**
   * Set the username/password of the request in the request attributes if they exist. Else, throw
   * an exception.
   * 
   * @param request
   * @return
  ...
  private void setResponseError(Object jsonRpcRequestId, Response response, String errorMessage) {
    ...
    try {
      ...
      LOGGER.error("Failed to send an error response to {}.", response.getRequest().getRemoteAddr(),
          e);
    }
  }
