...
public class StratumWorkerConnection extends StratumConnection implements WorkerConnection {
  ...
  private Boolean logRealShareDifficulty =
      ConfigurationManager.getInstance().getLogRealShareDifficulty();
  ...
  public void startReading() {
    ...
    subscribeTimeoutTask = new Task() {
      public void run() {
        LOGGER.warn("No subscribe request received from {} in {} ms. Closing connection.",
  ...
  protected void onParsingError(String line, Throwable throwable) {
    LOGGER.error("Parsing error on worker connection {}. Failed to parse line {}.",
  ...
  protected void onGetVersionRequest(ClientGetVersionRequest request) {
    LOGGER.warn("Worker {} send a GetVersion request. This should not happen.",
        getConnectionName());
  }
  ...
  public void onPoolSubmitResponse(MiningSubmitRequest workerRequest,
    ...
    if (logRealShareDifficulty) {
      Double realDifficulty = DifficultyUtils.getRealShareDifficulty(currentHeader, extranonce1Tail,
          workerRequest.getExtranonce2(), workerRequest.getNtime(), workerRequest.getNonce());
  ...
  protected void onAuthorizeResponse(MiningAuthorizeRequest request,
    ...
    LOGGER.warn("Worker {} send an Authorize response. This should not happen.",
        getConnectionName());
  }
  ...
  protected void onSubscribeResponse(MiningSubscribeRequest request,
    ...
    LOGGER.warn("Worker {} send a Subscribe response. This should not happen.",
        getConnectionName());
  }
  ...
  protected void onExtranonceSubscribeResponse(MiningExtranonceSubscribeRequest request,
    ...
    LOGGER.warn("Worker {} send an Extranonce subscribe response. This should not happen.",
        getConnectionName());
  }
  ...
  /**
   * Send the first notifications to the worker. The setDifficulty and the current job.
   */
   ...
   * setExtranonce is not supported.
   * 
   * @param newPool
  ...
  private void updateBlockHeader(MiningNotifyNotification notification) {
    ...
    // is bound to a pool.
    if (pool != null) {
      currentHeader = new GetworkJobTemplate(notification.getJobId(),
          notification.getBitcoinVersion(), notification.getPreviousHash(),
          notification.getCurrentNTime(), notification.getNetworkDifficultyBits(),
          notification.getMerkleBranches(), notification.getCoinbase1(),
          notification.getCoinbase2(), getPool().getExtranonce1() + extranonce1Tail);
      currentHeader.setDifficulty(pool.getDifficulty(),
          ConfigurationManager.getInstance().isScrypt());
    }
  }
  ...
  private boolean checkTarget(MiningSubmitRequest request) {
    ...
    if (currentHeader != null) {
      ...
      if (ConfigurationManager.getInstance().isScrypt()) {
        isShareValid = !ScryptHashingUtils.isBlockHeaderScryptHashBelowTarget(
            jobTemplate.getData(request.getExtranonce2()).getData(),
            currentHeader.getTargetInteger());
      } else {
        isShareValid = !SHA256HashingUtils.isBlockHeaderSHA256HashBelowTarget(
            jobTemplate.getData(request.getExtranonce2()).getData(),
            currentHeader.getTargetInteger());
      }
    }
