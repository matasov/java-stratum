...
public class Launcher {
	...
	private static void checkCertificate() throws Exception {
		...
		if (!storeFile.exists()) {
			...
			Certificate certificate = new CertificateFactory().engineGenerateCertificate(
					new ByteBufferBackedInputStream(ByteBuffer.wrap(certificateHolder.toASN1Structure().getEncoded())));
